...
export default class Player extends Component {
	...
	seek(time) {
		...
		this.audioElement.current && this.audioElement.current.seek(time);
		this.setState({
	...
	onBack() {
		if (this.state.currentPosition < 10 && this.state.selectedTrack > 0) {
			this.audioElement.current && this.audioElement.current.seek(0);
			...
			this.audioElement.current.seek(0);
			this.setState({
				currentPosition: 0,
			});
		}
